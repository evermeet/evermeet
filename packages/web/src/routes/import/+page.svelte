...
<div class="page-wide">
  ...
  <form class="flex gap-2 w-full" on:submit={formSubmit}>
    <div class="grow">
      <input
        ...
        disabled={isLoading}
        placeholder="External URL of event or calendar"
        bind:value={url}
      />
    </div>
    <div>
  ...
  <div class="mt-2 text-xs opacity-50">
    ...
    <a
      href="#"
      class="underline"
      on:click={() => submitExample("https://lu.ma/uncloud")}>lu.ma (event)</a
    >,
    <a
      href="#"
      class="underline"
      on:click={() => submitExample("https://lu.ma/berlin-blockchain-week")}
      >lu.ma (calendar)</a
    >,
    <a
      href="#"
      ...
      on:click={() =>
        submitExample(
          "https://www.meetup.com/paralelnipolis/events/300284599/",
        )}>meetup.com</a
    >
  </div>
  {#if error}
    ...
    <div id={results.url} class="mt-8">
      ...
      <div>
        <button class="btn btn-primary" disabled={selected.length === 0}
          >Import {selected.length} Events</button
        >
      </div>
    </div>
  {/if}
