...
<div class="page-wide">
  ...
  <div class="flex items-center">
    <div class="grow">
      <h2 class="heading2 no-margin">{$t`My Calendars`}</h2>
    </div>
    <div>
      <a href="/create-calendar" class="btn btn-sm btn-neutral"
        >+ {$t`Create Calendar`}</a
      >
    </div>
  </div>
    ...
    {#each subscribed as c}
      <div class="mb-3 itembox flex gap-8">
        <div class="w-[12rem]">
          ...
          {#if c._remote}
            <div class="badge badge-neutral font-mono text-xs my-2">
              {c._remote}
            </div>
          {/if}
          <a href={c.baseUrl} class="btn btn-sm mt-4 btn-neutral"
            >{$t`View calendar`} →</a
          >
        </div>
        <div class="">
