<script>
  ...
  async function submitMessage() {
    ...
    try {
      res = await xrpcCall(
        { fetch, user },
        "app.evermeet.chat.createMessage",
        null,
        {
          room: room.id,
          msg: input.value,
  ...
  function onKeyDown(e) {
    const isSearch = Boolean(
      window.document.getElementById("masterSearchDialog"),
    );
    if (isSearch) {
      return false;
    }
...
<div class="itembox no-padding xchat relative">
  <div
    class="absolute w-full h-20 bg-gradient-to-b from-base-300 to-transparent z-10"
  ></div>
        ...
        <div class="w-52 shrink-0 flex justify-end">
          <div class="">
            <div class="flex gap-1.5 items-center mb-auto mt-1 justify-end">
              <div class="font-bold break-words text-sm text-right">
                {m.author.name}
              </div>
              <div class="shrink-0">
            ...
            <div class="text-right text-xs opacity-50">
              {formatDistanceToNow(new Date(m.createdOn))} ago
            </div>
          </div>
        </div>
        <div class="grow opacity-90 prose prose-md max-w-none">
          {@html markdownToHTML(m.msg)}
        </div>
      </div>
    {/each}
  </div>
  ...
  {#if user}
    <form
      class="pb-1.5 px-1.5 flex items-center relative"
      on:submit|preventDefault={submitMessage}
    >
      <div
        class="rounded-l-lg bg-base-100 py-3 pl-3 pr-1.5"
        class:bg-base-200={isLoading}
      >
        <UserAvatar {user} />
      </div>
      <input
        id="xchat-input"
        type="text"
        class="input w-full !outline-none !border-0 pl-1.5 ml-0 rounded-l-none"
        bind:this={input}
        autocomplete="off"
      ...
      <button
        type="submit"
        class="absolute right-4 btn btn-sm btn-neutral"
        disabled={isLoading}><PaperAirplane size="16" /></button
      >
    </form>
  {/if}
</div>
