...
export default function ({ evermeet }) {
  ...
  return {
    async init() {
      ...
      for (const ep of evermeet.endpoints.list) {
        ...
        app[method](url, async (req, reply) => {
          let out = {};
          const input =
            ep.lex.defs.main.type === "procedure" ? req.body : req.query;
          const encoding = req.headers["content-type"] || "application/json";
          out = await evermeet.request(ep.id, {
            input,
            encoding,
            headers: req.headers,
            session: req.cookies[evermeet.config.api.sessionName],
          });
          if (out.error) {
            return reply
              .code(501)
              .send({ error: out.error, message: out.message });
          }
          return reply.send(out.body);
        });
        console.log(
          `[fastify] Route created: [${method.toUpperCase()}] ${url}`,
        );
      }
    ...
    async start() {
      return app.listen(
        { port: evermeet.config.api.port, host: evermeet.config.api.host },
