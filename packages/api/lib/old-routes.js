...
export function makeRoutes(app, api) {
  ...
  app.get("/api/explore", async (req, reply) => {
    const calendarsQuery = await api.cols.calendars
      .find({
        selector: {
          personal: { $ne: true },
        },
  ...
  app.post("/api/login", async (req, reply) => {
    ...
    const user = await api.cols.users
      .findOne({ selector: { emailShas: { $in: [hash] } } })
      .exec();
  ...
  app.post("/api/me/calendarSubscribe", async (req, reply) => {
    ...
    const subscribedCalendars = (user.subscribedCalendars || []).concat([
      { ref: calendarId, time: new Date().toISOString() },
    ]);
    await user.update({
      $set: {
        subscribedCalendars,
  ...
  app.post("/api/me/register", async (req, reply) => {
    ...
    const user = req.user;
    const item = await api.cols.events
      .findOne({ selector: { id: req.body.id } })
      .exec();
    if (!item) {
      return reply.code(400).send({ error: "wrong argument" });
    }
  ...
  app.post("/api/me/unregister", async (req, reply) => {
    ...
    let outEvent;
    const eventRegisterItem =
      item.guestsNative && item.guestsNative.find((e) => e.ref === user.did);
    ...
    const registerItem =
      user.events && user.events.find((e) => e.ref === item.id);
