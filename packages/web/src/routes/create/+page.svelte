<script>
  ...
  const calendar = writable({
    value: data.calendars[0].id,
    label: data.calendars[0].id,
  });
...
<div class="page-wide">
    ...
    <div class="w-full">
      ...
      <div class="mt-8 bg-base-200 rounded-lg">
        <div class="px-6 py-1 flex gap-4">
          <ul
            class="timeline timeline-vertical timeline-snap-icon timeline-compact opacity-75"
          >
            <li class="">
        ...
        <div class="py-1.5 px-5 w-full">
          <div
            class="text-sm text-base-content/25 w-full flex items-center gap-2"
          >
  ...
  <pre
    class="pre font-mono p-4 bg-base-200 text-sm w-full text-base-content">{stringify(
      output,
    )}</pre>
</div>
