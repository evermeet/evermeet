...
export async function load({ params, fetch, parent, url }) {
    ...
    const rooms = await xrpcCall({ fetch, user: data.user }, 'app.evermeet.chat.listRooms')
    ...
    const messages = (await xrpcCall({ fetch, user: data.user }, 'app.evermeet.chat.getMessages', { repo: room.repo, room: room.id }))?.reverse()
