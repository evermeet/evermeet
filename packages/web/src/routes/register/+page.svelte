...
<script>
    import { UserPlus, Check, ExclamationCircle, ArrowPath, Bolt, Key } from 'svelte-heros-v2';
    ...
    registerData.subscribe((d, p) => {
        validHandle = d.handle && d.handle.match(/^[0-9a-zA-Z-]+$/) && d.handle.length >= 3
        validData = validHandle && d.password && d.passwordRepeat && (d.password === d.passwordRepeat)
...
<div class="w-[28rem] m-auto my-24 itembox p-6 shadow-xl">
    ...
    <form class="mt-6" on:submit|preventDefault={submitRegistration}>
        ...
        <div class="flex items-center">
            ...
            {#if !customDomainMode}
                <div class="align-right text-xs opacity-50 hover:opacity-100"><button class="hover:underline flex gap-1.5 items-center active:opacity-75" on:click|preventDefault={generateHandle}><ArrowPath tabindex="-1" size="16" /> Random handle</button></div>
            {/if}
        </div>
        <div class="mt-2 mb-2">
        ...
        <div class="text-base-content/75 flex gap-3 items-center">
            ...
            <div><button class='text-xs flex gap-1.5 items-center hover:underline {customDomainMode ? 'text-primary' : 'opacity-50'} hover:opacity-100' on:click|preventDefault={toggleCustomDomainMode}><input type="checkbox" class="toggle toggle-xs toggle-primary" checked={customDomainMode ? 'checked' : false} on:click|preventDefault={() => {this.checked = !this.checked}} /> Custom domain</button></div>
        </div>
                ...
                <div class="py-3 px-4 bg-base-300 rounded-lg grid grid-cols-1 gap-2">
                    ...
                    <div>
                        ...
                        <div class="relative mt-2">
                            <div class="{$did ? '' : 'blur select-none h-10'} font-mono text-md">{$did?.did || "did=did:plc:123456789_123456_8901234"}</div>
                            {#if !$did}
                                <div class="absolute btn btn-sm btn-primary top-0 left-1 flex gap-1.5" on:click|preventDefault={generateDid}><Key /> Generate new DID</div>
                            {/if}
                        </div>
                    </div>
                </div>
        ...
        {:else}
            <div class="py-2 px-3 itembox mb-4 text-sm text-base-content/75 grid grid-cols-1 gap-2">
                ...
                <div class="flex gap-2 items-center">
                    <div>{#if $registerData.handle?.length >= 3}<div class="text-success"><Check tabindex="-1" /></div>{:else}<div class="text-error"><ExclamationCircle tabindex="-1"  /></div>{/if}</div>
        ...
        <div class="{validHandle ? '' : 'opacity-50'}">
            ...
            <div class="mt-2 mb-2">
                <input id="password" type="password" placeholder="Password" class="input input-bordered {isProcessing || !validHandle ? 'input-disabled' : ''} w-full" bind:value={$registerData.password} />
            </div>
            <label for="passwordRepeat" class="">Password (repeat)</label>
            <div class="mt-2">
                <input id="passwordRepeat" type="password" placeholder="Password" class="input input-bordered {isProcessing || !validHandle ? 'input-disabled' : ''} w-full" bind:value={$registerData.passwordRepeat} />
            </div>
        </div>
        <div class="mt-4">
            <button class="btn btn-primary {!validData || isProcessing ? 'btn-disabled' : ''} w-full">
                {#if !isProcessing}
                    Register
                {:else}
