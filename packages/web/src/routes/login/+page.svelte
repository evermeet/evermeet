<script>
  ...
  $: validCredentials =
    ...
    (!localInstance || (localInstance && $credentials.password?.length >= 3));

  credentials.subscribe((x) => {
    const domain = x.identifier
      ? x.identifier?.split(".").slice(1).join(".").toLowerCase()
      : null;
    if (domain) {
  ...
  async function submitLogin() {
    ...
    if (!localIdent) {
      const resp = await xrpcCall(
        { fetch },
        "app.evermeet.identity.resolveHandle",
        { handle: ident },
      );
      console.log(resp);
      isProcessing = false;
...
<div class="w-[26rem] m-auto my-24 itembox p-6 shadow-xl">
  ...
  <div class="mt-2 text-base-content/75">
    Please sign in below or <a
      href="/register"
      class="underline hover:no-underline">sign up</a
    >.
  </div>
  ...
  <form class="mt-6" on:submit|preventDefault={submitLogin}>
    ...
    <div class="mt-2 mb-2 relative">
      <input
        id="identifier"
        type="text"
        placeholder="your-handle{$instance === 'Other Instance'
          ? '.my-instance.com'
          : ''}"
      ...
      <span
        ...
        >{$instance === "Other Instance" ? "" : "." + $instance}</span
      >
    </div>
    {#if showPassword}
    ...
    <div class="mt-4">
      <button
        class="btn btn-primary {!validCredentials || isProcessing
          ? 'btn-disabled'
          : ''} w-full"
      >
        {#if !isProcessing}
          Login {#if $instance != $config.domain}<span class="font-mono"
              >on {$instance}</span
            >{/if}
        {:else}
