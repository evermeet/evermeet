        ...
        {#each items as item}
          <li use:melt={$option({ value: item.id, label: item.id })}>
            <div class="flex gap-2 justify-between items-center">
              <div>
                <a class="flex gap-2 text-base justify-left"><img src={item.img} class="w-5 h-5 rounded" />{item.name}</a>
              </div>
              <CheckCircle variation="solid" class="size-4 mr-2 {$isSelected(item.id) ? 'block' : 'hidden'}" />
            </div>
          </li>
        {/each}
