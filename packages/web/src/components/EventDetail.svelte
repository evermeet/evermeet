<script>
  ...
  $: countryName = item.placeCountry
    ? countries.getName(item.placeCountry, "en")
    : "";
  $: userRegistered =
      ...
      <div class="mt-4 mb-8">
        {#each item.hosts as host}
          <div class="flex gap-2 mb-2">
            <div class="w-6 h-6">
              <img class="rounded-full w-6 h-6 aspect-square" src={host.img} />
            </div>
            <div>{host.name}</div>
          </div>
        {/each}
    ...
    <div class="flex gap-4 items-center">
      <div class="w-10 h-10 border rounded-lg border-neutral">
        <div class="text-center">
          <div class="text-xs uppercase bg-neutral">
            {format(item.dateStart, "MMM")}
          </div>
      ...
      <div>
        <div class="text-lg font-mono">
          {format(item.dateStart, "eeee, MMMM d")}
        </div>
      ...
      <div class="mt-6 itembox no-padding w-full">
        ...
        <div class="py-4 px-4">
          {#if userRegistered}
            ...
            <div class="mt-4 text-sm text-base-content/75">
              No longer able to attend? Notify the host by <button
                class="underline text-accent opacity-75 hover:opacity-100"
                on:click={unregister(item.id)}
                >canceling your registration</button
              >.
            </div>
          {:else}
            <div class="mb-4">
            ...
            <div class="mt-3">
              <button
                ...
                on:click={register(item.id)}>Register</button
              >
            </div>
          {/if}
        </div>
