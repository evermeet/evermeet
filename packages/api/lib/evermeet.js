...
export class Evermeet {
  constructor(opts = { configFile: "../../config.yaml" }) {
    ...
    if (runtime.name !== this.config.api.runtime) {
      throw new Error(
        `Wrong runtime! in config: ${this.config.api.runtime}, current: ${runtime.name}`,
      );
    }

    this.logLevel = this.config.api.trace
      ? "trace"
  ...
  async init() {
    try {
      ...
      // load http adapter
      this.adapterMake = (
        await import(this.paths.adapters + this.config.api.adapter + ".js")
      ).default;
  ...
  async start() {
    ...
    this.logger.info(
      {
        adapter: this.config.api.adapter,
  ...
  async compileXrpcEndpoints(_ep) {
    ...
    this.logger.debug(
      { endpoints: list.map((e) => e.id) },
      `XRPC endpoints compiled (${list.length})`,
    );
    return { list, struct };
  }
  ...
  async request(id, { input, encoding, headers, session }) {
    ...
    try {
      ...
      // run handler
      this.logger.debug(
        { endpoint: id, user: authData?.user?.did || "nil" },
        "API request",
      );
      out = await endpoint.handler({
        input,
        encoding,
        ...authData,
        db,
  ...
  async objectGet(ctx, id, opts = {}) {
    ...
    if (id.match(/:/)) {
      ...
      const calendar = await ctx.db.calendars.findOne({
        $or: [
          { handle: calendarDid },
          { handle: `${calendarDid}.${this.config.domain}` },
        ],
      });
      if (calendar) {
    ...
    for (const c of Object.keys(cols)) {
      const found = await ctx.db[cols[c]].findOne({
        $or: [
          { handle: id },
          { handle: `${id}.${this.config.domain}` },
          { id },
          { did: id },
        ],
      });
